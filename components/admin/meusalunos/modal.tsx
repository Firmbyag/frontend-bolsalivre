...
const Modal: React.FC<ModalProps> = ({ newData, setShowModal }) => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const result = await axios.get(
          `${process.env.NEXT_PUBLIC_BACKEND_DEV}/api/users`
        );
        if (result) {
    ...
    const fetchTipos = async () => {
      try {
        const result = await axios.get(
          `${process.env.NEXT_PUBLIC_BACKEND_DEV}/api/tipo`
        );
        if (result) {
    ...
    const registerStudentAsync = async () => {
      try {
        ...
        const result = await axios({
          method: "post",
          url: `${process.env.NEXT_PUBLIC_BACKEND_DEV}/api/students/add`,
          data: data,
        });
        if (result) {
          console.log(result);
    ...
    <div className="top-0 left-0 fixed w-screen h-screen flex justify-center items-center z-30">
      ...
      <div className="absolute flex flex-col gap-5 border border-slate-300 rounded-lg p-5 bg-white">
        <div className="flex flex-col gap-2">
          ...
          <input
            type="text"
            className="rounded-full px-5 py-1 border border-slate-300 focus:outline-none focus:ring focus:ring-purple-300"
        ...
        <div className="flex flex-col gap-2">
          ...
          <select
            ...
            name=""
            id=""
            defaultValue={schools && schools.length > 0 && schools[0]._id}
            value={data && data.school && data.school}
            onChange={(e) => {
            ...
            {schools &&
              schools.map((school: any, index: number) => (
                <option key={index} value={school._id}>
                  <p>{`${school.level.level}-${school.series.series}`}</p>
                  <p>{`(${school.title})`}</p>
                </option>
              ))}
          </select>
        ...
        <div className="flex flex-col gap-2">
          ...
          <select
            ...
            name=""
            id=""
            defaultValue={users && users.length > 0 && users[0]._id}
            value={data && data.parent && data.parent}
            onChange={(e) => {
        ...
        <div className="flex flex-col gap-2">
          ...
          <select
            ...
            name=""
            id=""
            defaultValue={years && years.length > 0 && years[0].year}
        ...
        <div className="flex flex-col gap-2">
          ...
          <select
            ...
            name=""
            id=""
            defaultValue={tipos && tipos.length > 0 && tipos[0]._id}
            value={data && data.tipo && data.tipo}
            onChange={(e) => {
        ...
        <button
          className="bg-purple-500 py-2 text-white text-xl font-semibold rounded-full"
