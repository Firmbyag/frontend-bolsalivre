...
const CustomSelect = <T,>({ className, filterName, placeholder, items, value, setItem, renderItem }: CustomSelectProps<T>) => {  
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <input
        ...
        className={`${
          className && className
        } py-2 rounded-full w-full max-w-xs focus:outline-none`}
        placeholder={placeholder || `Selecione ${filterName}`}
      />
      {isDropdownVisible && (
        <ul className="absolute z-10 border border-slate-400 bg-white w-full max-w-xs rounded-lg shadow-lg max-h-60 overflow-x-hidden">
          {!isEmpty(filteredItems) ? (
            ...
            <li className="px-5 py-2 font-light text-xs text-gray-500">
