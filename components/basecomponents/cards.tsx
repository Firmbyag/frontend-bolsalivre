...
const TransformingCard: React.FC<TransformingCardProps> = ({
  ...
  return (
    <div className="flex flex-col justify-between w-full h-auto gap-5 p-5 bg-white border border-orange-300 rounded-lg">
...
const OtherSchoolsCard: React.FC<OtherSchoolsCardProps> = ({
  ...
  return (
    <Link
      href={`/artigos/${title}`}
      className="flex flex-col p-5 bg-white rounded-lg justify-between gap-5"
    >
...
const PrivateSchoolCard: React.FC<PrivateSchoolCardProps> = ({
  ...
  return (
    <Link
      href={`/artigos/${title}`}
      className="flex flex-col p-5 bg-white rounded-lg justify-between w-60 gap-2 
    border border-slate-400"
    >
      <div className="flex md:flex-row items-center gap-1 md:gap-0">
        <SchoolImage title={title} />
        <p className="text-[16px] font-semibold text-gray-700 md:ml-3 md:text-xs text-center md:text-left">
          {title}
        </p>
      </div>
  ...
  return (
    <div className="flex flex-col space-y-5">
      ...
      <div className="flex flex-col w-full  items-start space-y-5">
        ...
        <button className="text-orange-500 font-bold border-[2px] rounded-full p-3 px-5 hover:bg-orange-500 hover:text-white border-orange-500">
          LEIA MAIS {"->"}{" "}
        </button>
      </div>
    </div>
...
const CategoriesCard: React.FC<CategoriesCardProps> = ({ name }) => {
  ...
  return (
    <>
      ...
      <ul className="flex flex-col text-orange-500 space-y-6">{cateLi}</ul>
    </>
  );
};
  ...
  if (school) {
    return (
      <div className="flex flex-col gap-3 p-5 border border-slate-300 rounded-lg shadow-inner">
        <span className="text-xl font-semibold">{school.title}</span>
        ...
        <div className="flex gap-1">
          <span className="text-slate-400 line-through">
            {school.scholarUnit} {school.amount}
          </span>
          <span className="text-slate-700">
            {school.scholarUnit} {school.monthlyState}
          </span>
        </div>
        <p>
          <svg
          ...
          Desconto de 40% nas mensalidades
        </p>
        <p>
          <svg
            className="float-start mx-1"
        ...
        <Link
          className="bg-orange-500 rounded-full p-3 text-white text-xl text-center font-semibold"
          href={`/escola/${school.title}/1`}
        >
          Quero esta bolsa
        </Link>
        <EscolaRegisterCard
...
const EscolaRegisterCard: React.FC<EscolaRegisterCardProps> = ({
  ...
  return (
    <div
      className={`${
        show ? "fixed" : "hidden"
      } top-0 left-0 flex w-[100vw] h-[100vh] justify-center items-center z-50`}
    >
      <div
        className="w-full h-full bg-slate-900 opacity-75"
